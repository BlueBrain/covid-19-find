...
def update_system_params(p, fixed_params):
   ...
   remaining_pop=fixed_params['total_pop']-high_risk_urban-hosp_staff
   woh=remaining_pop*fixed_params['prop_15_64']*fixed_params['prop_woh'] #working outside home
   other_hc=high_risk_urban+woh
...
def process_scenarios(country_df,p,scenarios,initial_beta):
   ...
   for i in range(0,num_scenarios):
      ...
      past=create_past(p,scenario_default,[1, 40, 46, 88],[0.07,0.18,1.0,0.96])
      p.update(past)
      ...
      raw_max_betas = get_beta(no_intervention_betafile, num_compartments)
...
class Par:
   ...
   def __init__(self,params):
      ...
      self.tau = self.infection_fatality_rate/self.death_period
...
class Sim:
   def __init__(self,num_days,num_compartments):
      ...
      self.infected4recover=np.zeros((num_days,num_compartments))
      ...
      self.isolated4recover=np.zeros((num_days,num_compartments))
 ...
 #it would be better to make this proportional to population
       ...
       prop_infections_in_compart=par.init_pop/np.sum(par.init_pop)
   ...
   def perform_tests(sim,par:Par,i,t,phase,use_real_testdata):
      truepositives=0
      falsepositives=0
      truenegatives=0
      falsenegatives=0
 ...
 #     print('user_real_testdata',use_real_testdata,'ispast',ispast(par.day1,t),'is_counterfactual',par.is_counterfactual[phase])
      if (use_real_testdata) and ispast(par.day1,t) and not (par.is_counterfactual[phase]):
          tests_available=sim.totaltestsperformed_mit[t]*par.prop_tests[i][phase]
      ...
      if tests_available>0:
         ...
         if par.test_symptomatic_only[phase]: 
            ...
            if sim.population[t-1,i]>0:
                ...
                falsepositives=falsepositives+tests_performed * sim.infectednotisolated[t-1-par.incubation_period,i]/sim.population[t-1,i] * (1-par.specificity[phase])
            else:
                truepositives=0
                falsepositives=0
   ...
   def get_data_frame(self,num_days,num_compartments,compartment):
      df = pd.DataFrame({
          ...
          'isolated4recover':np.round(self.isolated4recover[:,0],1),
          'infected': np.round(self.infected[:,0],1),
          'infected4recover':np.round(self.infected4recover[:,0],3),
      ...
      for i in range(1,num_compartments):
         dfadd = pd.DataFrame({
            ...
            'infected4recover':np.round(self.infected4recover[:,i],3),
...
#       print('t=',t,'contacts per person',contacts_per_person,'contacts per person isolated=',contacts_per_person_isolated)
       for i in range(0,par.num_compartments):
           ...
           (truepositives,falsepositives,accum_tests_performed) = sim.perform_tests(par,i,t,phase,use_real_testdata)  
           ...
           #newisolatedinfected is underestimated
           sim.newisolatedinfected[t,i] = truepositives#this does not yet take account of contacts or of delays in getting results
           sim.newconfirmed[t,i] = sim.newisolated[t,i]
           if t-(par.recovery_period)>=0:
               sim.newrecovered[t,i] = sim.infected4recover[t-1,i]*par.gamma
               sim.infected4recover[t,i]=sim.infected4recover[t-1,i]+sim.newinfected[t-par.recovery_period,i]-sim.newrecovered[t,i]   
               if sim.infected4recover[t,i]<0:
                   sim.infected4recover[t,i]=0
           else:
           ...
           if t-par.recovery_period>=0:
               newisolatedrecovered=sim.isolated4recover[t-1,i]*par.gamma #+sim.isolated[t-1-par.death_period,i]*par.tau
               sim.isolated4recover[t,i]=sim.isolated4recover[t-1,i]+sim.newisolated[t-par.recovery_period,i]-newisolatedrecovered+sim.isolated[t-1-par.death_period,i]*par.tau
               if sim.isolated4recover[t,i]<0:
                  sim.isolated4recover[t,i]=0
           else:
               sim.isolated4recover[t,i]=0
               newisolatedrecovered=0
           sim.isolated[t,i]=sim.newisolated[t-1,i]+sim.newisolated[t,i]-newisolatedrecovered
           if sim.isolated[t,i]<0:
               sim.isolated[t,i]=0
           sim.isolatedinfected[t,i] = sim.isolatedinfected[t-1,i] + sim.newisolatedinfected[t,i] - sim.isolatedinfected[t-1-par.recovery_period,i]*par.gamma-sim.isolatedinfected[t-1-par.death_period,i]*par.tau
