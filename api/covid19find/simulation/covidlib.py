...
class Sim:
 ...
 #it would be better to make this proportional to population
   ...
   # tests are performed for symptomatic patients first - if there are any  left over they are used to test asymptomatics
        ...
        p_infected_symptomatic=infected_symptomatic_population/symptomatic_population
   ...
   def perform_tests_with_priorities(sim,par:Par,t,phase,use_real_testdata,priorities):
       ...
       #symptomatic and asymptomatic subjects are both tested
       # a symptomatic subjects are retested every x days where x depends on par.retest_period_asymptomatics
       # if the number of tests is low only health workers will get tested
        infected_symptomatic_population=np.zeros(par.num_compartments)
        uninfected_symptomatic_population=np.zeros(par.num_compartments)
        symptomatic_tests=np.zeros(par.num_compartments)
        asymptomatic_tests=np.zeros(par.num_compartments)
        infected_symptomatic_tests=np.zeros(par.num_compartments)
        infected_asymptomatic_tests=np.zeros(par.num_compartments)
        symptomatic_population=np.zeros(par.num_compartments)
        simphase,today=computetoday(par.day1,par.trig_values)
        p_infected=np.zeros(par.num_compartments)
        infected_symptomatic_population=sim.newinfected[t-par.incubation_period]*(1-par.prop_asymptomatic)
   #     infected_asymptomatic_population=sim.newinfected[t-par.incubation_period]*(par.prop_asymptomatic)
        uninfected_symptomatic_population=sim.population[t-1]*par.background_rate_symptomatic
    #    uninfected_population=sim.susceptibles[t]-sim.infected[t]
        symptomatic_population=infected_symptomatic_population+ uninfected_symptomatic_population
        p_infected_symptomatic=infected_symptomatic_population/symptomatic_population
        asymptomatic_population=sim.population[t-1]-symptomatic_population
        asymptomatic_eligible_testing=asymptomatic_population/par.retest_period_asymptomatics
    ...
    #asymptomatics are tested periodically (period in retest_period_asymptomatics)
                eligible_testing=symptomatic_population[i] +asymptomatic_eligible_testing[i]
                if eligible_testing> total_tests_available:
                    symptomatic_tests[i]=total_tests_available*symptomatic_population[i]/eligible_testing
                    asymptomatic_tests[i]=total_tests_available*asymptomatic_eligible_testing[i]/eligible_testing
                else:
                    symptomatic_tests[i]=symptomatic_population[i]
                    asymptomatic_tests [i]=asymptomatic_eligible_testing[i]
                infected_symptomatic_tests[i]=symptomatic_tests[i]*p_infected_symptomatic[i]
                infected_asymptomatic_tests[i]=infected_symptomatic_tests[i]*(par.prop_asymptomatic)/(1-par.prop_asymptomatic)
                tests_performed[i]=symptomatic_tests[i]+asymptomatic_tests[i]
                p_infected[i]=(infected_symptomatic_tests[i]+infected_asymptomatic_tests[i])/tests_performed[i]
   ...
   # This is meant to simulate testing on a first come first served basis
   # We assume that a certain % of people who come in for testing are symptomatic
