 ...
 #it would be better to make this proportional to population
   ...
   def perform_tests_with_priorities(sim,par:Par,t,phase,use_real_testdata,priorities):
       ...
       # if the number of tests is low only health workers will get tested
        ...
        uninfected_symptomatic_population=sim.population[t]*par.background_rate_symptomatic
        symptomatic_population=infected_symptomatic_population+ uninfected_symptomatic_population
        asymptomatic_population=sim.population[t]-symptomatic_population
        ...
        patients=symptomatic_population+asymptomatic_population/par.retest_period_asymptomatics
        for k in range(0, par.num_compartments):
            i=priorities[k]
            # everyone else is either uninfected symptomatic or uninfected asymptomatic
            if total_tests_available>patients[i]:
                tests_performed[i]=patients[i]
            if tests_performed[i]>uninfected_symptomatic_population[i]:
                    uninfected_symptomatic_tests[i]=uninfected_symptomatic_population[i]
            else:    
                    uninfected_symptomatic_tests[i]=tests_performed[i]*par.background_rate_symptomatic
            if tests_performed[i]>symptomatic_population[i]:
                infected_symptomatic_tests[i]=symptomatic_population[i]-uninfected_symptomatic_tests[i]
            else:
                infected_symptomatic_tests[i]=tests_performed[i]-uninfected_symptomatic_tests[i]
            if tests_performed[i]>0:
                p_infected[i]=infected_symptomatic_tests[i]/tests_performed[i]
            else:
                p_infected[i]=0  
   ...
   def perform_tests_open_public(sim,par:Par,t,phase,use_real_testdata):
       ...
       # patients=symptomatic_population+asymptomatic_population/par.retest_period_asymptomatics
        for i in range(0, par.num_compartments):
            # everyone else is either uninfected symptomatic or uninfected asymptomatic
# =============================================================================
#             if tests_available[i]>patients[i]:
#                 tests_performed[i]=patients[i]
# =============================================================================
            uninfected_symptomatic_tests[i]=tests_performed[i]*par.background_rate_symptomatic
